...
model = T5ForConditionalGeneration.from_pretrained(modelInput)
...
for i in range(1,6):
    txtFile = outTestFile[:-4]+str(i)+".txt"
    #Removing older version of the file
    if os.path.exists(txtFile):
        os.remove(txtFile)
    testFiles1.append(txtFile)
...
for i in range(1,6):
    txtFile = "CodeT5_Test_cases_{0}.txt".format(i)
    #Removing older version of the file
    if os.path.exists(txtFile):
        os.remove(txtFile)
