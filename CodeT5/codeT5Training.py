...
def generate_output(text,tokenizer,model,max_len=200):
...
if __name__ == '__main__':
    ...
    tb_logger = pl_loggers.TensorBoardLogger(save_dir="codeT5_logs/")
    checkpoint = ModelCheckpoint(dirpath="checkpoint_codeT5_3/")
    if(epochsTakenEn!=0):
        model = CodeT5(tokenizerA=tokenizer,dataModule=summary_data_en,model=model)
        # model= nn.DataParallel(model)
        # model.to(device)
        trainer = Trainer(gpus=2,
                             default_root_dir="/CodeT5/EnCheckpoints",
    ...
    if(epochsTakenCode!=0):
        ...
        # model.to(device)
        trainer = Trainer(gpus =2,
                             default_root_dir="/CodeT5/CodeCheckpoints",
    ...
    if(epochsTaken!=0):
        ...
        # model.to(device)
        trainer = Trainer(gpus = 2,
                             default_root_dir="/CodeT5/ModelCheckpoints",
    ...
    for i in tqdm(range(len(testData)), desc="Task Completed: External Test Output"):
        input = testData[i]
        new_test = generate_output(input,tokenizer,model,max_len=500)
        #Writing to file
