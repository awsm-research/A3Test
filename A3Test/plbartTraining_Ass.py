...
class CodeT5(pl.LightningModule):
    def __init__(self,tokenizerA, dataModule, model, lr=2e-5, num_train_epochs=15, warmup_steps=1000):
        super().__init__()
        self.model = model
...
class SummaryDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        train_testvalid = datasetWhole['train'].train_test_split(test_size=0.9)
    ...
    def setup(self):
        ...
        self.train_test_valid_dataset = self.train_test_valid_dataset.map(preprocess_examples, batched=True)
...
def generate_output(text,tokenizer,model, max_len=250):
...
if __name__ == '__main__':
    if(epochsTakenEn!=0):
        ...
        summary_data_en = SummaryDataModule(tokenizer, enPreTrainInput,
                                         batch_size = 32, num_examples = 62400002001)
    ...
    if(epochsTakenCode!=0):
        ...
        summary_data_code = SummaryDataModule(tokenizer, codePreTrainInput,
                                         batch_size = 32, num_examples = 62400002100)
    ...
    tb_logger = pl_loggers.TensorBoardLogger(save_dir="plBart_logs/")
    checkpoint = ModelCheckpoint(dirpath="checkpoint_plBart_3/")
    if(epochsTakenEn!=0):
        model = CodeT5(tokenizerA=tokenizer,dataModule=summary_data_en,model=model)
        # model= nn.DataParallel(model)
        # model.to(device)
        trainer = Trainer(gpus=1,
                             default_root_dir="/plBart/EnCheckpoints",
    ...
    if(epochsTakenCode!=0):
        ...
        # model.to(device)
        trainer = Trainer(gpus =1,
                             default_root_dir="/plBart/CodeCheckpoints",
    ...
    if(epochsTaken!=0):
        ...
        # model.to(device)
        trainer = Trainer(gpus = 1,
                             default_root_dir="/plBart/ModelCheckpoints",
    ...
    for i in tqdm(range(len(testData)), desc="Task Completed: External Test Output"):
        input = testData[i]
        new_test = generate_output(input,tokenizer,model,max_len=250)
        #Writing to file
