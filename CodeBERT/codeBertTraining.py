...
if __name__ == '__main__':
    ...
    tb_logger = pl_loggers.TensorBoardLogger(save_dir="codeBert_logs/")
    checkpoint = ModelCheckpoint(dirpath="checkpoint_codeBert_3/")
    if(epochsTakenEn!=0):
        model = CodeT5(tokenizerA=tokenizer,dataModule=summary_data_en,model=model)
        # model= nn.DataParallel(model)
        # model.to(device)
        trainer = Trainer(gpus=2,
                             default_root_dir="/codeBert/EnCheckpoints",
    ...
    if(epochsTakenCode!=0):
        ...
        # model.to(device)
        trainer = Trainer(gpus =2,
                             default_root_dir="/codeBert/CodeCheckpoints",
    ...
    if(epochsTaken!=0):
        ...
        # model.to(device)
        trainer = Trainer(gpus = 2,
                             default_root_dir="/codeBert/ModelCheckpoints",
